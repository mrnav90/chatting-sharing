...
/**
 * @abstract Transport selection option key.
 * @discussion This key allows selecting the transports that the Hype
 * SDK should enable. This allows the developer control over which 
 * transports are used for network discovery and communications. The 
 * available transports are those listed under HYPTransportType. All
 * drivers except HYPTransportTypeWeb are available for iOS and macOS,
 * and tvOS supports the same except HYPTransportTypeBluetoothLowEnergy.
 * All those drivers will be made available soon. This key expects a
 * bitwise enumeration of transports to enable. This means that in
 * order to specify several transports the bitwise OR operator is to
 * be used. For instance, in order to specify that the SDK should start
 * Bluetooth Low Energy and Infrastructural Wi-Fi, pass 
 * "[NSNumber numberWithInt:HYPTransportTypeBluetoothLowEnergy | HYPTransportTypeWiFiInfra]"
 * as the value for this option when starting Hype's services. Notice,
 * however, that some of the drivers work in bundles. On all Apple
 * platforms, Bluetooth Classic, Infrastructural Wi-Fi, and Wi-Fi 
 * Direct (peer-to-peer Wi-Fi) use Bonjour to function and as such
 * they cannot be started individually. This means that specifying
 * any of those drivers also requests the others to start. These
 * three transports must either all run or none. The default value
 * for this is HYPTransportTypeAll, meaning that if this option is
 * not set all drivers are requested to start. This option can be
 * passed to -startWithOptions: or -setOption:forKey: to Hype's
 * singleton instance.
 */
FOUNDATION_EXPORT NSString * const HYPOptionTransportKey;
...
/**
 * @abstract Announcement option key.
 * @discussion Announcements help identifying devices on the network.
 * When an instance is found, its identifier might not be enough for
 * the user or the application to identify the user behind the device.
 * Although vendor identifiers already help with this, announcements
 * are an extra space for identification purposes. The SDK is oblivious
 * to the type of data passed on an announcement, but it optimizes its 
 * delivery by caching it on the network. This is better than sending
 * a message as soon as two devices find each other, especially in
 * multi-hop scenarios. Still, announcements are very limited. At most,
 * an announcement can be 255 bytes long. This value is expected to be
 * extended in future versions, but at this point the feature is 
 * experimental.
 */
FOUNDATION_EXPORT NSString * const HYPOptionAnnouncementKey;
 ...
 * with Hype's singleton instance property -state.
 */
 ...
 * This class wraps the domestic instance created for the host device.
 ...
 * not be re/Users/ze/Desktop/Hype/Repositorio/hype.ios/Hype/HYP.mgistered twice, preventing the observer from getting duplicate
 * notifications.
...
/**
 * @abstract Sets an option for Hype's services.
 * @discussion This method allows setting options for the Hype SDK. When
 * the services are started with the -start method, options set by this
 * method are used. When -startWithOptions: is used instead, options set
 * with this method are also used, but options passed to that method
 * take precedence and override the previous option. All options set
 * persist through several calls to the start family of methods.
 ...
 * devices. It works similarly to -startWithOptions:, except that it
 * works with options that have been previously set. Such options can
 * be set by either a previous call to -startWithOptions: or
 * -setOption:forKey:.
 */
- (void)start;
 ...
 * that point, it's useless trying to start the services again. Instead,
 ...
 * and allow for fine tuning of the framework's behaviour. Future releases
 * will keep adding more options for further configuration abilities.
 ...
 * for other instances. This does not imply previously found instances
 ...
 * two concepts are important to distinguish: sending and delivering. A
 ...
 * may not be the same as the one the data is intended to. The trackProgress
